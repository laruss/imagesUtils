...
from description.settings import (
    DescriptionSettings,
    Methods,
    GPTModel,
    Service,
...
def add_arguments():
    ...
    parser.add_argument(
        "--gpt-model",
        help="gpt model",
        choices=[model.name for model in GPTModel],
        default=settings.gpt_model.name,
    )
    parser.add_argument(
        "--used-gpt",
        help="used gpt",
        choices=[service.name for service in Service],
    ...
    DescriptionUtils(
        settings=DescriptionSettings(
            ...
            gpt_settings=GPTSettings(
                model=GPTModel[args.gpt_model], service=Service[args.used_gpt]
            ),
        )
