...
def _get_data(
    query: str = None, options: Options = None
) -> Dict[Literal["options", "context"], dict]:
    ...
    return {"options": options.model_dump(mode="json"), "context": {}}
...
def _get_model(data: Dict[Literal["options", "context"], dict]) -> Model:
    ...
    logger.info(
        f"Fetching items from pinterest.com with query '{data['options']['query']}'"
    )

    response = requests.get(
        URL, params={"source_url": source_url, "data": json.dumps(data)}, headers={}
    )

    assert (
        response.status_code == 200
    ), f"Error {response.status_code} when fetching items"
    resp_data = response.json()
    assert (
        resp_data["resource_response"]["status"] == "success"
    ...
    while len(processed_items) < limit:
        ...
        for item in model.resource_response.data.results:
            processed_items.append(
                ProcessedItem(id=item.id, title=item.title, media=item.images.orig.url)
            )
