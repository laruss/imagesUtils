...
def add_arguments():
    ...
    parser.add_argument(
        "--limit", help="limit of posts", type=int, default=settings.images_limit
    )
    parser.add_argument(
        "--prompt", help="prompt for request", type=str, default=settings.prompt
    )
...
def main():
    ...
    logger.info(
        f"Downloading images from `{args.source}` source, limit: {args.limit}, prompt: {args.prompt}"
    )

    DownloadUtils(
        settings=DownloadSettings(
            source=Sources[args.source], images_limit=args.limit, prompt=args.prompt
        ),
