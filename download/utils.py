...
def download_image(post: ProcessedItem) -> None:
    ...
    raw_image = None
    image_path = f"{CoreSettings().images_folder}/{post.id}.jpg"
...
def flow(settings: DownloadSettings = DownloadSettings()) -> None:
    ...
    for post in posts:
        all_posts_data = read_json_from_file(CoreSettings().data_file, False) or {}
